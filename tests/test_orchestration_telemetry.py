...
# Load the hook module by file path. We point at the canonical source in
# the .claude/shared submodule rather than the locally-symlinked
# .claude/hooks/ path, because the symlink layer is generated by
# scripts/claude-sync.sh (gitignored) and is not built in CI. The
# submodule path is fetched by actions/checkout submodules: recursive.
# Filename has a hyphen, which is invalid in module identifiers, so we
# load via spec_from_file_location and assign a hyphen-free name.
_HOOK_PATH = (
...
_spec = importlib.util.spec_from_file_location("telemetry_stamp", _HOOK_PATH)
