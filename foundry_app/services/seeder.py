...
def seed_tasks(
    ...
    if mode == "beans":
        return _seed_beans(composition, tasks_dir, result)
...
def _seed_beans(
    ...
    """Generate the beans backlog structure for a new project.

    Creates ``_index.md`` (master backlog index) and ``_bean-template.md``
    (template for creating new beans) inside *beans_dir*.
    """
    index_content = f"""\
# Bean Backlog: {composition.project.name}

> Generated by Foundry (beans mode). Use `/new-bean` to add beans.
...
"""
    ...
    index_file = beans_dir / "_index.md"
    index_file.write_text(index_content)
    result.wrote.append("_index.md")

    template_file = beans_dir / "_bean-template.md"
    template_file.write_text(template_content)
    result.wrote.append("_bean-template.md")
