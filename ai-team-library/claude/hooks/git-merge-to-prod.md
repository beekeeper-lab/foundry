...
Merges a feature branch to main/prod after approval, with safety gates requiring CI to be green and a minimum approval count. This is the final gate before production deployment.

## Hooks

| Hook Name | Trigger | Check | Pass Criteria | Fail Action |
|-----------|---------|-------|---------------|-------------|
| `ci-green-check` | `pre-merge` | Verify all CI checks on the feature branch are passing | All required status checks show success | Block merge; list failing checks |
...
| `prod-merge` | `post-approval` | Execute merge from feature branch to main/prod with `--no-ff` | Merge commit created on production branch | Block; report merge failure details |
