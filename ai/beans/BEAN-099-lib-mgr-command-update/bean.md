# BEAN-099: Library Manager — Command Update
...
In the Library Manager, users cannot edit and save commands. The existing code in library_manager.py has update logic for commands, but it does not work correctly end-to-end — items may not appear in the tree, operations may not persist, or the UI may not refresh properly.

## Goal

A user can select an existing command, edit its markdown content in the editor, save changes, and confirm the file on disk is updated.

## Scope

### In Scope
- Selecting a command loads its current content into the editor
...
Depends on BEAN-098 (Command Create) — need items to exist to test editing.
...
> Duration backfilled from git timestamps (commit→merge, 317s).
