# BEAN-091: Library Manager — Template Update
...
In the Library Manager, users cannot edit and save templates. The existing code in library_manager.py has update logic for templates, but it does not work correctly end-to-end — items may not appear in the tree, operations may not persist, or the UI may not refresh properly.

## Goal

A user can select an existing template, edit its markdown content in the editor, save changes, and confirm the file on disk is updated.

## Scope

### In Scope
- Selecting a template loads its current content into the editor
...
- [x] Selecting a template loads its content into the editor
...
Depends on BEAN-090 (Template Create) — need items to exist to test editing.
...
> Duration backfilled from git timestamps (commit→merge, 397s).
